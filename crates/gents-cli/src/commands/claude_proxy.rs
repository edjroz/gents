...
pub(crate) async fn wait_for_claude_proxy_healthz(bind: SocketAddr) -> Result<()> {
    ...
    loop {
        ...
        if std::time::Instant::now() >= deadline {
            bail!("Claude proxy did not become ready at {url} within 5 seconds: {last_error}");
        }
        tokio::time::sleep(Duration::from_millis(50)).await;
    }
}

/// Bind, spawn the accept loop, then wait for `/healthz`.
///
/// Spawn must happen before healthz. Waiting first deadlocks because the
/// accept loop is not running yet (A2a-2 hang in `36444a55`).
pub(crate) async fn start_managed_claude_proxy(
    config: ClaudeProxyConfig,
    ...
    let (info, serve) = bind_claude_proxy(config, shutdown).await?;
    ...
    if let Err(error) = wait_for_claude_proxy_healthz(info.bind).await {
        ...
        return Err(error).context("managed Claude proxy bound but failed healthz");
    }
    Ok((info, handle))
}
...
mod tests {
    ...
    #[tokio::test]
    async fn start_managed_spawns_before_healthz() {
        // Regression for A2a-2 hang in 36444a55: healthz before spawn never
        // becomes ready because the accept loop is not running.
        let temps = TempDirs {
            log_dir: tempfile::tempdir().unwrap(),
            workdir: tempfile::tempdir().unwrap(),
            config_dir: tempfile::tempdir().unwrap(),
        ...
        let config = ClaudeProxyConfig {
            ...
            port: 0,
            config_dir: temps.config_dir.path().to_path_buf(),
            workdir: Some(temps.workdir.path().to_path_buf()),
            log_dir: Some(temps.log_dir.path().to_path_buf()),
        ...
        let (info, handle) = start_managed_claude_proxy(config, shutdown_rx)
            .await
            .expect("start managed");
        assert!(
            started.elapsed() < Duration::from_secs(3),
            "managed start took {:?}; likely healthz-before-spawn hang",
            started.elapsed()
        );
        assert_eq!(info.mode, "canned");
